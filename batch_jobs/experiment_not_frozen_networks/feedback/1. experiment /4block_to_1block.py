...
# Experiment idea: train frozen and not frozen models in one run
...
if __name__ == "__main__":
    ...
    checkpoint_filepath_frozen = f'../../../../models/FrozenVGG16/5 epochs VGG16Feedback 4Block To 1Block/checkpoint'
    model_checkpoint_callback_frozen = tf.keras.callbacks.ModelCheckpoint(
    ...
    with strategy.scope():
        ...
        print("----------------------------------------------------------")
        model.VGG_before_feedback.trainable = True
        print("Train not frozen model")
        history = model.fit(train_ds, epochs=epochs, validation_data=valid_ds, callbacks=[checkpoint_filepath], verbose=1) 
